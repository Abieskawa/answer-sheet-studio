...
#. ``python scripts/e2e_demo.py --input test/八年級期末掃描.pdf``
#. 依照程式輸出提示開啟 ``http://127.0.0.1:8000/result/<job_id>/charts``
...
- The web UI supports **English**, **Traditional Chinese**, and **Simplified Chinese**.
...
- ``results.csv`` (student columns default to ``grade-class-seat`` like ``8-1-01``; duplicates get ``_2`` / ``_3``)
- ``ambiguity.csv``
- ``annotated.pdf``
...
- **上传处理（识别＋分析）**：上传多页 PDF（每页一位学生）＋老师答案档 ``answer_key.xlsx``。完成后会提供「打开结果页」按钮，并输出 ``results.csv``、``ambiguity.csv``、``annotated.pdf``，以及分析报表/图表（图表会依界面语言显示）。

使用注意事项
~~~~~~~~~~~~

- 进行图像识别时，**题数** 与 **每题选项（ABC/ABCD/ABCDE）** 必须与答案卡一致。
...
- 「试题分析数据（预览）」为可展开表格（预设收合）；需要完整资料请下载 ``analysis_item.csv``。
...
- 打开 ``http://127.0.0.1:8000/debug``，输入 Job ID（``outputs/`` 底下的资料夹名称）。
- 下载 ``results.csv``、``ambiguity.csv``、``annotated.pdf`` （必要时再下载 ``input.pdf`` ），提供给开发者协助排查。

疑难排解
~~~~~~~~

- 若看到 ``ERR_CONNECTION_REFUSED`` /「127.0.0.1 拒绝连接」，请再双击启动器（ ``start_mac.command`` / ``start_windows.vbs`` ）重新启动。
- **pip install failed** – 检查 launcher log，确认有网络、Python 版本为 3.10+；若你是 Python 3.14，建议改用 3.10–3.13。
- **Port already in use / permission denied** – 启动器会检查 port。
- **Server not opening** – 先看 launcher log 找出错误；也可以在终端机执行 ``python run_app.py`` （需先在同一环境安装 requirements）方便除错。
...
#. ``python scripts/e2e_demo.py --input test/八年級期末掃描.pdf``
#. 依照程式输出提示打开 ``http://127.0.0.1:8000/result/<job_id>/charts``
