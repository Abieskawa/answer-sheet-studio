...
# Auto-exit when the browser closes (mac default). Override via env vars.
export ANSWER_SHEET_AUTO_EXIT="${ANSWER_SHEET_AUTO_EXIT:-1}"
export ANSWER_SHEET_IDLE_TIMEOUT_SEC="${ANSWER_SHEET_IDLE_TIMEOUT_SEC:-60}"
export ANSWER_SHEET_IDLE_CHECK_SEC="${ANSWER_SHEET_IDLE_CHECK_SEC:-15}"
export ANSWER_SHEET_HEARTBEAT_SEC="${ANSWER_SHEET_HEARTBEAT_SEC:-20}"
...
if [ "${ANSWER_SHEET_OPEN_BROWSER:-1}" != "0" ]; then
  ( # subshell so we don't block Finder
    ...
    if [ "$found" -eq 0 ]; then
      ...
      open "http://127.0.0.1:${default_port}" >/dev/null 2>&1 || true
    fi
  ) >/dev/null 2>&1 &
fi
